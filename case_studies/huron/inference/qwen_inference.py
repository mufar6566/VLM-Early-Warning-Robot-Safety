...
def build_prompt(input_mode, sensor_context):
    base = """
You are a robot safety monitor for indoor navigation.
...
def score_labels(model, processor, inputs):
    ...
    if is_qwen_model(model):
        try:
            ...
            scores = {x: -1e9 for x in LABELS}
            probs = np.array([1/3, 1/3, 1/3], dtype=np.float64)
            return "safe", scores, probs
        except Exception as e:
            print("Qwen first-token scoring error:", str(e), flush=True)
            scores = {x: -1e9 for x in LABELS}
            probs = np.array([1/3, 1/3, 1/3], dtype=np.float64)
            return "safe", scores, probs
