...
def normalize_probs(p_safe, p_punsafe, p_unsafe):
    ...
    if not np.isclose(vals.sum(), 1.0, atol=1e-6):
        return np.full(len(LABELS), np.nan, dtype=np.float64)
...
def main():
    ...
    if args.skip_existing and out_csv.exists():
        try:
            ...
            expected = None if end_frame < 0 else end_frame - start_frame + 1
            if expected is not None and len(old) == expected:
    ...
    if df.empty:
        raise RuntimeError(
            f"No frames selected: start_frame={start_frame}, "
