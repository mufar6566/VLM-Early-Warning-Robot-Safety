...
def build_prompt(include_speed=False, speed_mps=None, angular_speed=None, min_laser_distance_m=None):
    base = """
...
""".strip()
    ...
    if include_speed:
        ...
        base += (
            "\n\nAdditional robot sensor context for the current frame:"
            f"\n- Linear speed: {_fmt_sensor(speed_mps, ' m/s')}"
            f"\n- Angular speed: {_fmt_sensor(angular_speed, ' rad/s')}"
            f"\n- Minimum LiDAR distance: {_fmt_sensor(min_laser_distance_m, ' m')}"
            "\nUse these values only as supporting context together with the visual frames."
        )
...
def main():
    ...
    parser.add_argument(
        "--gt_csv",
        default="../../02_ground_truth/bww1_Feb-16-2023-bww1-intloss_00000000_frame_level_gt_fisheye_dynamic_warning.csv",
        help="Frame-level GT CSV with dynamic warning targets.",
    )

    parser.add_argument(
        "--bag_dir",
        default=".",
        help="Bag root folder containing images/ and csv/.",
    )

    parser.add_argument(
        "--out_csv",
        default="../../03_predictions/llava_onevision_bww1_00000000_history_predictions.csv",
    ...
    if args.config_csv is not None:
        ...
        input_mode = str(
            config_value("input_mode", "visual")
        ).strip().lower()

        if input_mode == "visual":
            args.include_speed = False
        elif input_mode == "sensor":
            args.include_speed = True
        else:
            raise ValueError(
                f"Unsupported input_mode={input_mode!r}; "
                f"expected 'visual' or 'sensor'."
            )
    ...
    print("Max frame:", args.max_frame_id)
    print("Stride:", args.stride)
    print("Max video frames:", args.max_video_frames)
    print("Include speed:", args.include_speed)
    print("Generate raw:", args.generate_raw)

    df = pd.read_csv(gt_csv)
    df = df.sort_values("frame_id").reset_index(drop=True)

    if args.max_frame_id >= 0:
        run_df = df[
            (df["frame_id"] >= args.start_frame)
            & (df["frame_id"] <= args.max_frame_id)
            & ((df["frame_id"] - args.start_frame) % args.stride == 0)
        ].copy()
    else:
        run_df = df[
            (df["frame_id"] >= args.start_frame)
    ...
    for n, (_, row) in enumerate(run_df.iterrows(), start=1):
        ...
        history_paths = all_image_paths[: frame_id + 1]
        ...
        prompt = build_prompt(
            ...
            speed_mps=row.get("odom_speed_mps", None),
            angular_speed=row.get("odom_angular_speed", None),
            min_laser_distance_m=row.get("min_laser_distance_m", None),
        )
